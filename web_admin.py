...
<body>
    ...
    <div class="main-content">
        ...
        <div class="tab-pane-content" id="tab-products" style="display:none;">
            <div class="card p-4">
                <div id="prodFlash" class="py-2 px-3 small mb-3" style="display:none;background:#2a1466;border:1px solid #6d3fe0;border-radius:12px;"></div>
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5 class="m-0 fw-bold">📦 Manage Products</h5>
                    <button class="btn btn-custom btn-sm" onclick="openProdModal()"><i class="fas fa-plus me-1"></i> Add Product</button>
                </div>
                <div class="row g-3">
                    {% for p in products %}
                    <div class="col-md-6">
                        <div class="card p-3" style="background:#12072c;border:1px solid #3b1d6e;">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                {% if p.maintenance %}<span class="badge bg-warning text-dark">● MAINTENANCE</span>
                                {% elif p.stock_out %}<span class="badge bg-danger">● DISABLED</span>
                                {% else %}<span class="badge bg-success">● ACTIVE</span>{% endif %}
                            </div>
                            <small class="text-muted mt-2">PRODUCT</small>
                            <h5 class="fw-bold">{{ p.name }}</h5>
                            <div class="text-muted small">{% for pl in p.plans %}{{ pl.name }} - ₹{{ pl.price }}{% if not loop.last %} • {% endif %}{% else %}No plans{% endfor %}</div>
                            <div class="d-flex flex-wrap gap-2 mt-3">
                                <button class="btn btn-sm btn-outline-light" onclick="openProdModal('{{ p.prod_key }}')"><i class="fas fa-pen"></i> Edit</button>
                                <button class="btn btn-sm btn-outline-light" onclick="toggleProduct('{{ p.prod_key }}', 'disable')">{% if p.stock_out %}▶️ Enable{% else %}⏸️ Disable{% endif %}</button>
                                <button class="btn btn-sm btn-outline-light" onclick="toggleProduct('{{ p.prod_key }}', 'maintenance')">🛠️ {% if p.maintenance %}End Maintenance{% else %}Maintenance{% endif %}</button>
            ...
            <div class="modal-bg" id="prodModal">
                <div class="modal-card">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h5 class="fw-bold m-0 text-info" id="pm_title">➕ Add Product</h5>
                        <button class="btn-close btn-close-white" onclick="closeProdModal()"></button>
                    </div>
                    <input type="hidden" id="pm_key">
                    <label class="small text-muted">Product Name</label>
                    <input type="text" id="pm_name" class="form-control mb-3" placeholder="e.g. Netflix Premium">
                    <div class="row g-2 mb-3">
                        <div class="col-8">
                            <label class="small text-muted">Category</label>
                            <select id="pm_cat" class="form-select">
                        ...
                        <div class="col-4"><label class="small text-muted">Icon</label><input type="text" id="pm_icon" class="form-control" placeholder="⚡"></div>
                    </div>
                    <label class="small text-muted">Channel Link (optional)</label>
                    <input type="text" id="pm_link" class="form-control mb-3" placeholder="e.g. https://t.me/yourchannel">
                    <hr style="border-color:#3b1d6e;">
                    <div class="fw-bold mb-2">➕ Plans (optional)</div>
                    <div id="pm_plans"></div>
                    <button class="btn btn-sm btn-outline-light mb-3" onclick="addPlanRow()">➕ Add More Plan</button>
                    <button class="btn btn-custom w-100" onclick="saveProduct()">➕ Save Product</button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function flashReload(msg, tab) {
            try { sessionStorage.setItem('flash', JSON.stringify({msg: msg, tab: tab || 'products'})); } catch (e) {}
            location.reload();
        }
        window.addEventListener('DOMContentLoaded', () => {
            try {
                let f = JSON.parse(sessionStorage.getItem('flash') || 'null');
                if (!f) return;
                sessionStorage.removeItem('flash');
                let link = [...document.querySelectorAll('.sidebar-link')].find(a => (a.getAttribute('onclick') || '').includes("'" + f.tab + "'"));
                showTab(f.tab, link);
                let box = document.getElementById('prodFlash');
                box.textContent = f.msg; box.style.display = 'block';
        ...
        function addPlanRow(p) {
            ...
            d.className = 'plan-row p-3 mb-3';
            d.style.cssText = 'border:1px solid #3b1d6e;border-radius:14px;background:#0d0622;position:relative;';
            d.innerHTML = `
                <button type="button" class="btn-close btn-close-white" style="position:absolute;top:10px;right:10px;" onclick="this.parentElement.remove()"></button>
                <div class="row g-2 pe-4">
                    <div class="col-7"><input class="form-control pl_name" placeholder="e.g. 1 Day / 30 Days" value="${esc(p.name)}"></div>
                    <div class="col-5"><input type="number" class="form-control pl_price" placeholder="Price (₹)" value="${esc(p.price)}"></div>
                </div>
                <label class="small text-muted mt-2">🤝 Reseller Price (₹) (optional)</label>
                <input type="number" class="form-control pl_rprice" placeholder="optional" value="${esc(p.reseller_price)}">
                <div class="row g-2 mt-1">
                    <div class="col-6"><label class="small text-muted">Remote Product ID</label><input class="form-control pl_rpid" placeholder="e.g. PID_123" value="${esc(p.remote_pid)}"></div>
                    <div class="col-6"><label class="small text-muted">Remote Duration</label><input class="form-control pl_rdur" placeholder="e.g. 1 Day" value="${esc(p.remote_duration)}"></div>
                </div>
                <label class="small text-muted mt-2">Add keys (optional, one per line)</label>
                <textarea class="form-control pl_keys" rows="2" placeholder="KEY1&#10;KEY2"></textarea>`;
            document.getElementById('pm_plans').appendChild(d);
        }
        function openProdModal(k) {
            let p = k ? PRODUCTS.find(x => x.prod_key === k) : null;
            let sel = document.getElementById('pm_cat');
            if (p && ![...sel.options].some(o => o.value === p.category)) sel.add(new Option(p.category, p.category));
            document.getElementById('pm_title').textContent = p ? '✏️ Edit Product' : '➕ Add Product';
            document.getElementById('pm_key').value = p ? p.prod_key : '';
            document.getElementById('pm_name').value = p ? p.name : '';
            sel.value = p ? p.category : 'non_root';
            document.getElementById('pm_icon').value = p ? p.icon : '';
            document.getElementById('pm_link').value = p ? p.download_link : '';
            document.getElementById('pm_plans').innerHTML = '';
            if (p && p.plans.length) p.plans.forEach(pl => addPlanRow(pl)); else addPlanRow();
            document.getElementById('prodModal').style.display = 'flex';
        }
        function saveProduct() {
            let name = document.getElementById('pm_name').value.trim();
            if (!name) return alert('Enter product name!');
            let plans = [...document.querySelectorAll('#pm_plans .plan-row')].map(r => ({
                name: r.querySelector('.pl_name').value.trim(), price: r.querySelector('.pl_price').value,
                reseller_price: r.querySelector('.pl_rprice').value, remote_pid: r.querySelector('.pl_rpid').value.trim(),
                remote_duration: r.querySelector('.pl_rdur').value.trim(), keys: r.querySelector('.pl_keys').value
            })).filter(p => p.name || p.price !== '');
            if (plans.some(p => !p.name || p.price === '')) return alert('Each plan needs a name and a price!');
            fetch('/api/product/save', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({prod_key: document.getElementById('pm_key').value, name: name,
                    category: document.getElementById('pm_cat').value, icon: document.getElementById('pm_icon').value,
                    download_link: document.getElementById('pm_link').value.trim(), plans: plans})
            }).then(r => r.json().then(d => r.ok ? flashReload(d.message) : alert(d.message)));
        }
        function toggleProduct(k, field) {
            fetch('/api/product/toggle', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({prod_key: k, field: field}) })
            .then(r => r.json()).then(d => flashReload(d.message));
        }
...
def dashboard():
    ...
    for p in prod_rows:
        ...
        for pn, pp in parse_plans(p[3]):
            m = meta.get((p[0], pn))
            plans.append({"name": pn, "price": pp, "reseller_price": m[2] if m else None,
                          "remote_pid": (m[3] if m else "") or "", "remote_duration": (m[4] if m else "") or ""})
...
def api_save_p():
    ...
    if not name: return jsonify({"message": "Product name is required!"}), 400
    plans = []
    for p in d.get('plans', []):
        pn = str(p.get('name', '')).strip()
        if not pn and p.get('price') in ('', None): continue
        try:
            price = float(p.get('price'))
            rp = p.get('reseller_price')
            rprice = float(rp) if rp not in ('', None) else None
        except (TypeError, ValueError):
            return jsonify({"message": "Plan prices must be numbers!"}), 400
        plans.append({"name": pn, "price": price, "rprice": rprice, "rpid": (p.get('remote_pid') or '').strip(),
